...
public class JdbcUsuarioRepository implements UsuarioRepository{
	...
	private int saveUsuarioInfo(Usuario usuario) {
		PreparedStatementCreatorFactory pscf = new PreparedStatementCreatorFactory("INSERT INTO users(mail, pass) VALUES (?, ?)", Types.VARCHAR, Types.VARCHAR);
		pscf.setReturnGeneratedKeys(true);
		PreparedStatementCreator psc = pscf.newPreparedStatementCreator(Arrays.asList(usuario.getMail(), usuario.getPass()));
	...
	public Usuario findOne(String mail, String pass) {
		...
		try {
			usuario = jdbc.queryForObject("SELECT * FROM users WHERE mail = ? AND pass = ?", this::mapRowToUser, mail, pass);
		} catch (Exception e) {
